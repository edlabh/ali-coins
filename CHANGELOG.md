...
- **Lockfile: renovação por `mtime` (`lockfile.js`):** o refresh periódico sobrescrevia o lock por `rename` incondicional após checar a posse — outra geração assumida na janela era sobrescrita. Agora a renovação atualiza apenas o **mtime** do lock (atômica, sem reescrever o conteúdo e sem janela de ausência) e o critério de stale considera `createdAt` **ou** `mtime` (timestamp inválido/futuro continua stale imediato, anti-DoS). A carência de leitura passou a valer também para `ENOENT`, para que uma remoção/renovação momentânea não seja tratada como "lock inexistente" e um segundo processo assuma a execução.
...
- **Multi-conta: "Saldo Final" correto (`libs/report.js`):** reutiliza `computeFinalBalance` normalizando o sufixo — não imprime mais `N/D moedas` nem `N moedas moedas`.
