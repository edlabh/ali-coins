...
- **Execução com `--user <uid>:<gid>` do host:** Se você especificar o usuário do host para manter as permissões dos arquivos montados, monte `/etc/passwd` e `/etc/group` em modo somente-leitura para que a resolução de usuário do Node (`os.userInfo()` via libuv `uv_os_get_passwd`) não gere erro `ENOENT`:
  ...
  docker run --rm \
    ...
    ali-coins
  ```
- **Compose e wrapper de produção:** use `docker-run.example.sh` (wrapper de cron com medição de pico de memória/PIDs) ou `docker-compose.yml` (limites declarativos: `shm_size: 256m`, tmpfs `/tmp`, `cap_drop: ALL`, `no-new-privileges`, memória 768M) para não repetir as flags manualmente.
