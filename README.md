...
- **Hosts com 1 GB de RAM (Oracle Free Tier, t2/t3.micro, DO):** recomende 1–2 GB de swap no host (`fallocate -l 2G /swapfile`) e limite o container: `--init --pids-limit=256 --shm-size=256m --memory=768m --memory-swap=1536m`. As flags de baixo consumo do Chromium já são aplicadas por padrão (desative com `CHROMIUM_LOW_MEMORY=false`) e `SCRYPT_N=32768` reduz o pico do scrypt de ~134 MB para ~33 MB. Detalhes em [`CLOUD_SESSIONS.md`](CLOUD_SESSIONS.md#7-otimizações-para-vps-com-pouca-memória-512-mb---1-gb-ram).
