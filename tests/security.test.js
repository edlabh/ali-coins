...
test(
  'security.js - safeWriteFile no fallback não segue symlink (não sobrescreve alvo)',
  { skip: process.platform === 'win32' ? 'symlink exige privilégio no Windows' : false },
  async () => {
    const fs = require('fs');
...
test(
  'security.js - safeChmod600 não altera permissão do alvo de um symlink',
  { skip: process.platform === 'win32' ? 'chmod POSIX não se aplica no Windows' : false },
  () => {
    const fs = require('fs');
    const path = require('path');
    const os = require('os');
